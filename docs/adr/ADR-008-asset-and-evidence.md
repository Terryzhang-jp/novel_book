...
## 更正 A2 与 A8（2026-08-03 · Commit 15C）

### 音频不再是「上传支持、发布跳过」

A8 原本只描述了图片的安全派生。音频当时没有实现，publishWork 会跳过它
并把计数报给用户。那个状态持续了三个提交，期间存在一个语义断裂：
**作者在草稿里看到的和读者在发布页看到的不是同一份内容。**

现在音频走同一条原则，只是手段不同：

```
图片   sharp 重新编码 → WebP，长边 1600，不带元数据
音频   ffmpeg 重新编码 → Opus/Ogg，单声道 48kHz 64kbps，不带元数据
```

**重新编码是剥离元数据唯一可靠的方式。** 逐个删已知标签字段挡不住
容器的私有扩展段 —— 格式很多，扩展更多，漏掉一个就是漏掉。
解码成 PCM 再编回去之后，能穿过来的只有声音本身。

一并丢掉的还有两样容易被忽略的东西：章节（里面有标题文字）、
内嵌封面（那是一张完整的图片，它自己就带 EXIF 和 GPS）。

### 派生副本按类型有不同的必填项

原来 `published_assets.width/height` 是 NOT NULL，这逼着音频行填一个
假的 1×1。「为了让字段有值而编一个值」是这个项目明确反对的做法
（ADR-009 的同一条原则）。

改成按 preset 分别约束：

```
web1600        必须有宽高，必须没有时长
audio_opus64   必须有时长，必须没有宽高
```

由 `chk_published_asset_shape` 保证。少了它，渲染端只能靠 mime_type
猜自己该读哪几列 —— 那正是 timezone 那一列的教训。

### 预设名里带参数

`audio_opus64` 而不是 `audio`。改码率、改声道数都要换一个**新的预设名**，
否则已经发布的旧副本会被追溯解释成新参数。

第一版是单声道：语音速记够用，环境录音的空间感会丢失。
这是已知取舍，记在 `verification-gaps.json` →
`audio-single-channel-loses-space`，解法是加新预设而不是改旧的。
